...
-- [回答]
SELECT store_name, good_name, quantity
FROM stock_table AS st
INNER JOIN store_table AS s ON st.store_code
INNER JOIN goods_table AS gt st.goods_code = gt.goods_code;
...
WHERE quantity > (SELECT AVG (quantity) FROM stock_table WHERE stock_code = 'EA01';
